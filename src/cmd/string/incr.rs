...
/// `Incr` command for string type.
/// `Incr` 命令用于字符串类型。
///
/// Increases the value of the specified key by a given step, with no default value.
/// 将指定键的数值增加指定的步长，无默认值。
/// If the key does not exist, a new key is created with the value of 1.
/// 如果键不存在，新建一个键，值为 1。
...
impl Incr {
    ...
    /// # Return
    /// # 返回
    /// Returns the parsed `Incr` instance containing the key and step. / 返回解析后的 `Incr` 实例，包含键和步长。
    fn parse_command(parse: &mut Parse) -> crate::Result<Self> {
        let key = parse.next_string()?;  // Get the key to be incremented / 获取要增加的键
        // Default step is 1 / 默认步长为 1
        let step = 1;
